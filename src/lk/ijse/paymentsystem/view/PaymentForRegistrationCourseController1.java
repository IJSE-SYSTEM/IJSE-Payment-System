 ...
 * @author midda
 */
public class PaymentForRegistrationCourseController1 {
...
//    private Connection c;
    ...
    public PaymentForRegistrationCourseController1(StudentDTO studentDTO, CourseDetailsDTO cdto, RegistrationDTO rdto) {
        this.course = cdto;
...
//         c=ConnectionFactory.getInstance().getConnection();
    ...
    public PaymentForRegistrationCourseController1(String sid, CourseDetailsDTO cdto, RegistrationDTO rdto) {
        this.course = cdto;
        this.batchID = rdto.getBatchId();
        this.rdto = rdto;
        this.sid = sid;
//         c=ConnectionFactory.getInstance().getConnection();
    ...
    private void getStudent() {
        ...
        try {
            ...
            Logger.getLogger(PaymentForRegistrationCourseController1.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public ArrayList<DefaultTableModel> setPaymentScheme() {
        int semester = course.getNo_of_Semesters();
        
        DefaultTableModel tblMdlPay1 = new DefaultTableModel(new String[][]{{"Full Payment"}}, new String[]{"Semesters"}){
            public boolean isCellEditable(int rowIndex, int columnIndex) {
                return false;
            }
        };
        
        DefaultTableModel tblMdlPay2 = new DefaultTableModel(new String[]{"Portions"}, 0){
            public boolean isCellEditable(int rowIndex, int columnIndex) {
                return false;
            }
        };
        ...
        for (int i = 0; i < semester; i++) {
            tblMdlPay1.addRow(new String[]{"Semester "+(i+1)});
            tblMdlPay2.addRow(new String[]{"Semester "+(i+1)+" Portion 1"});
            tblMdlPay2.addRow(new String[]{"Semester "+(i+1)+" Portion 2"});
        }
        
        ArrayList<DefaultTableModel> tblMdls=new ArrayList<>(2);
        tblMdls.add(tblMdlPay1);
        tblMdls.add(tblMdlPay2);
        return tblMdls;
    }
